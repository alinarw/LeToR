...
def rmse(y, p, w, lamb, n):
    return np.sqrt(2 * (loss(y, p) + reg(w, lamb)) / n)
...
def sgd(x, y, lamb, lr_low, lr_high, mu, sigma):
    w = np.zeros((len(mu) + 1, 1))
    print(w.shape)
    Phi = phi(x, mu, sigma)
    Phi = np.hstack((np.ones((Phi.shape[0], 1)), Phi))
    print(Phi.shape)
    print(Phi[9, :].reshape((-1, 1)).shape)
    n = x.shape[0]
    for i in range(x.shape[0]):
        ...
        if i % 100:
            print(lr)
        tmp1 = w.T.dot(Phi[i, :])
        tmp = (y[i, :] - tmp1) * (Phi[i, :].reshape((-1, 1)))
...
if __name__ == '__main__':
    ...
    #sigma = [np.random.uniform(0, 1, (46,46)), np.random.uniform(0, 1, (46,46))]
    ...
    # SGD
    # w = sgd(x_train, y_train, lamb, lr_low, lr_high, mu, sigma)
    #
    # p = linear(x_test, w, mu, sigma)
    # print(w)
    # rmse0 = rmse(y_test, p, w, lamb, y_test.shape[0])
